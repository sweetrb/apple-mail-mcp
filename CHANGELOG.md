...
- **The forensic stream is framed against its own contents.** The values it carries come from inbound mail — the RFC Message-ID always, the subject under `APPLE_MAIL_MCP_AUDIT_SUBJECTS` — so the control characters the stream is delimited with are stripped out of every interpolated value at the source, in the same AppleScript that reads them, and out of the account/mailbox names interpolated from TypeScript. Without that, a crafted Message-ID containing a record separator plus a forged `RECON` tag could inject a reconciliation record and fabricate the exact `over` warning this feature exists to produce — reachable only with the audit log enabled, which is precisely when someone is chasing a real incident and can least afford invented evidence. Stripped characters are replaced with `U+FFFD` rather than dropped, so an altered value shows that it was altered.
...
- **`errors` reached only two of the three outcomes.** When *every* id failed — the outcome a caller most needs to act on — the response was text-only, so nothing appeared in `structuredContent` at all. All three outcomes now carry the same structured payload.
