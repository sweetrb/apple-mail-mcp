...
export class AppleMailManager {
  ...
  renameMailbox(oldName: string, newName: string, account?: string): boolean {
    ...
    if (result.output.startsWith("partial")) {
      ...
      const total = parts[2] ?? "?";
      console.error(
        `Failed to rename mailbox: only ${parts[1] ?? "?"} of ${total} messages moved, ${remaining} remain in "${resolvedOld}"; source was NOT deleted (both mailboxes left intact). Retry to move the rest.`
      );
