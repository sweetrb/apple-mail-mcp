...
- Optional services: SMTP, IMAP, Contacts, smart-mailbox, and filesystem-attachment features are disabled unless the host/configuration enables them. The default AppleScript mail path is local to the Mac, but it still has the Mail.app authority granted by macOS TCC.
...
| TM-007 — plaintext SMTP/IMAP downgrade             | Closed         | #165 requires STARTTLS for non-implicit-TLS transports and documents the explicit plaintext escape hatch.                                             |
| TM-008 — unpinned runtime package                  | Open           | The plugin still resolves the npm package through `npx`; pinning/integrity binding is separate work.                                                  |
