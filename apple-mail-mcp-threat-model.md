...
- Intended use: an MCP server used to control personal or organizational Apple Mail from an AI host. The user selected a maximum-security posture and asked that broader or remote client exposure be considered even though this repository's native entrypoint is stdio.
- Deployment assumption: a host or wrapper may expose the stdio MCP to more than one client or to a network-facing service. The repository contains no authentication, tenant isolation, or authorization layer for such a wrapper; the wrapper must supply it.
...
- Does the actual MCP host expose this stdio process to remote or multi-user clients, and if so what authenticates and authorizes each client?
- Are SMTP, IMAP, Contacts, smart-mailbox, and filesystem attachment features enabled in the production profile, or is the deployment read-only AppleScript by default?
- Which local directories are intended for outbound attachments and inbound saves, and can they be dedicated per workflow rather than the whole home directory?
...
- `codex/.mcp.json` and the plugin manifests define how a host starts the server. The Codex manifest currently resolves `apple-mail-mcp` from npm at runtime.
...
- MCP host -> local filesystem attachment readers: absolute path strings cross into `existsSync`, `readFileSync`, AppleScript attachment commands, or Nodemailer attachment descriptors. Existing validation requires absolute existing paths but does not impose a narrow read-root policy.
- Mail.app or IMAP -> local filesystem attachment writers: email-controlled filenames and bytes cross into `saveAttachment` or the IMAP save handler. Save roots and traversal/symlink checks exist, but an existing regular destination can still be overwritten.
- `src/index.ts` -> `imapClient.ts` -> IMAP server: account labels, mailbox paths, UIDs, searches, attachment sections, and mutations cross an authenticated TLS-or-STARTTLS boundary. IMAP ids encode account, path, and UID, but attachment buffering is not bounded and non-implicit-TLS configuration is not forced to upgrade.
- `src/index.ts` -> `smtpMailer.ts` -> SMTP server: recipient data, message content, local attachment paths, and Keychain-backed credentials cross an outbound network boundary. Nodemailer is configured with `secure`, but the runtime does not set `requireTLS` for STARTTLS mode.
...
- Plugin install -> npm registry -> launched server: `codex/.mcp.json` invokes `npx -y apple-mail-mcp` without an exact version or integrity binding. The plugin manifest version and the executable resolved by npm are therefore separate trust claims.
...
| IMAP attachment fetch     | `fetch-attachment`, IMAP save path                 | IMAP provider to process memory          | BODYSTRUCTURE size is available but buffer growth is unbounded              | `src/services/imapClient.ts` `streamToBuffer`, `imapFetchAttachment`                       |
...
| Runtime supply chain      | Plugin install and MCP start                       | Registry/plugin to executable            | `npx` resolves an unpinned package                                          | `codex/.mcp.json`                                                                          |
...
6. **Exhaust the MCP process:** a provider advertises or streams a very large attachment -> `streamToBuffer` retains every chunk and then allocates a concatenated buffer and base64 representation -> memory pressure degrades or kills the server.
...
| `codex/.mcp.json`                            | Runtime package resolution is separate from plugin manifest version                                  | TM-008                         |
